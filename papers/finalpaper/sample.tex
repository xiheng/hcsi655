...
Figure \ref{fig:arch} presents our proposed system architecture. Mobile users can draw sketches together despite their lack of temporal co-location. For example, user A drew sketch in a certain location (point$_{0}$) at t$_{0}$ and left the location. The user A's drawing has been stored into database through web service (web server). Sometime later, user B came to the location (point$_{0}$) and adds some sketches into the previous sketch drawn by user A at t$_{1}$. Web users can view what people has drawn by their web browser which accesses to our web service (web server). Each marker in our map page indicates there are sketches in the location.
...
stack that it has to offer.  In particular, we has used the following:
...
\end{itemize}
 ...
 We built up a google map with markers of sketches on a web page. This enables users view sketches globally and have a sense of the sketch distribution directly.
